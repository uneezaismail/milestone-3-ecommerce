    ...
    >
      <div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden md:flex items-center space-x-10">
            {menuItems.map((item, index) => (
              <Link
                key={index}
                href={item.link}
                className="text-white font-medium text-lg hover:border-b-indigo-500 hover:border-b"
              >
                {item.name}
              </Link>
            ))}
          ...
          <ul className="flex items-center gap-x-4 sm:gap-x-6 relative">
            {icons.map((item, index) => (
              <Link key={index} href={item.link}>
                <li className="p-1 font-poppins hover:bg-white rounded-full text-white cursor-pointer hover:text-blue-800 relative">
                  {/* Add cart badge if totalItems > 0 */}
                  {item.icon}
                  {item.link === "/cart" && totalItems > 0 && (
                    <span className="absolute -top-2 -right-3 bg-red-600 text-white text-[10px] rounded-full px-2 py-1">
                      {totalItems}
                    </span>
                  )}
                </li>
